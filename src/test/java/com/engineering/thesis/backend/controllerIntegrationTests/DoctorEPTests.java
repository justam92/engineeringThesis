...
public class DoctorEPTests extends MockConfiguration {
    ...
    @Test
    public void getByIdShouldReturnDoctorWhenProperRoleIsSelected() throws ResourceNotFoundException {
        System.out.println("Running test -> " + Thread.currentThread().getStackTrace()[1].getMethodName());
        final Long id = 1L;
        when(doctorServiceImpl.getById(id)).thenReturn(Doctors.doctor1);
        RulesMap().forEach((name, Role) -> {
            try {
                System.out.println("Performing getById doctor with role: " + name);
                if (name.equals("Doctor") || name.equals("Patient") || name.equals("Manager") || name.equals("Admin")) {
                    this.mockMvc
                            .perform(MockMvcRequestBuilders.get("/api/doctors/1").with(Role)
