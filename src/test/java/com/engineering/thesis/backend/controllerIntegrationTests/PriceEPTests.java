...
class PriceEPTests extends MockConfiguration {
    ...
    @Test
    public void getAllShouldReturnPricesWhenProperRoleIsSelected() {
        System.out.println("Running test -> " + Thread.currentThread().getStackTrace()[1].getMethodName());
        when(priceServiceImpl.getAll()).thenReturn(List.of(Prices.price1, Prices.price2));
        RulesMap().forEach((name, Role) -> {
            try {
                ...
                if (name.equals("Doctor") || name.equals("Patient") || name.equals("Manager") || name.equals("Admin")) {
                    this.mockMvc
                            .perform(MockMvcRequestBuilders.get("/api/prices").with(Role))
    ...
    @Test
    public void getByIdShouldReturnPriceWhenProperRoleIsSelected() throws ResourceNotFoundException {
        System.out.println("Running test -> " + Thread.currentThread().getStackTrace()[1].getMethodName());
        final Long id = 1L;
        when(priceServiceImpl.getById(id)).thenReturn(java.util.Optional.of(Prices.price1));
        RulesMap().forEach((name, Role) -> {
            try {
                System.out.println("Performing getById price with role: " + name);
                if (name.equals("Doctor") || name.equals("Patient") || name.equals("Manager") || name.equals("Admin")) {
                    this.mockMvc
                            .perform(MockMvcRequestBuilders.get("/api/prices/1").with(Role))
                            .andExpect(status().isOk())
    ...
    public void createPriceShouldBePossibleWhenProperRoleIsSelected() {
        ...
        RulesMap().forEach((name, Role) -> {
            try {
                ...
                if (name.equals("Manager") || name.equals("Admin")) {
                    this.mockMvc
                            .perform(
                                    post("/api/prices")
                            ...
                            .andExpect(status().isOk());
                    verify(priceServiceImpl, atLeast(1)).create(any(PriceRequest.class));
    ...
    @Test
    public void deleteByIdPriceShouldBePossibleWhenProperRoleIsSelected() {
        System.out.println("Running test -> " + Thread.currentThread().getStackTrace()[1].getMethodName());
        RulesMap().forEach((name, Role) -> {
            try {
                System.out.println("Performing deleteById price with role: " + name);
                if (name.equals("Manager") || name.equals("Admin")) {
                    this.mockMvc
                            .perform(delete("/api/prices/1").with(Role))
                            .andExpect(status().isOk());
                    System.out.println("Status 200 received properly as expected \n");
                }
                if (name.equals("Invalid") || name.equals("Doctor") || name.equals("Patient")) {
    ...
    @Test
    public void updateByIdPriceShouldBePossibleWhenProperRoleIsSelected() {
        System.out.println("Running test -> " + Thread.currentThread().getStackTrace()[1].getMethodName());
        RulesMap().forEach((name, Role) -> {
            try {
                System.out.println("Performing updateById price with role: " + name);
                if (name.equals("Manager") || name.equals("Admin")) {
                    this.mockMvc
                            .perform(
                                    put("/api/prices/1")
                            ...
                            .andExpect(status().isOk());
                    verify(priceServiceImpl, atLeast(1)).updateById(anyLong(), any(Price.class));
                    System.out.println("Status 200 received properly as expected \n");
                }
                if (name.equals("Invalid") || name.equals("Doctor") || name.equals("Patient")) {
                    this.mockMvc
